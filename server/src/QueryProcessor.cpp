...
nlohmann::json QueryProcessor::executeStatement(const string&st,QueryContext&ctx){string n=Parser::upper(Parser::trim(st));nlohmann::json r;if(n.rfind("CREATE DATABASE",0)==0){string d=Parser::parseNameAfter(st,"CREATE\\s+DATABASE");storage_.createDatabase(d);r["message"]="Database created: "+d;return r;}if(n.rfind("SET DATABASE",0)==0){string d=Parser::parseNameAfter(st,"SET\\s+DATABASE");if(!storage_.databaseExists(d))throw runtime_error("Database does not exist: "+d);ctx.database=d;r["message"]="Database context set to: "+d;return r;}if(n.rfind("SELECT",0)==0){auto q=Parser::parseSelect(st);if(Parser::upper(q.tableName).rfind("SYSTEM",0)!=0)requireDb(ctx);return storage_.selectRows(ctx.database,q);}if(n.rfind("CREATE TABLE",0)==0){requireDb(ctx);string t=Parser::parseCreateTableName(st);storage_.createTable(ctx.database,t,Parser::parseCreateTableColumns(st));r["message"]="Table created: "+t;return r;}if(n.rfind("DROP TABLE",0)==0){requireDb(ctx);string t=Parser::parseDropTableName(st);storage_.dropTable(ctx.database,t);r["message"]="Table dropped: "+t;return r;}if(n.rfind("INSERT INTO",0)==0){requireDb(ctx);string t=Parser::parseInsertTableName(st);storage_.insertRow(ctx.database,t,Parser::parseInsertValues(st));r["message"]="1 row inserted into "+t;return r;}if(n.rfind("CREATE INDEX",0)==0){requireDb(ctx);storage_.createIndex(ctx.database,Parser::parseIndexName(st),Parser::parseIndexTable(st),Parser::parseIndexColumn(st),Parser::parseIndexType(st));r["message"]="Index created";return r;}if(n.rfind("DELETE",0)==0){requireDb(ctx);string t=Parser::parseNameAfter(st,"FROM");int x=storage_.deleteRows(ctx.database,t,Parser::parseWhere(st));r["message"]=to_string(x)+" rows deleted";return r;}if(n.rfind("UPDATE",0)==0){requireDb(ctx);int x=storage_.updateRows(ctx.database,Parser::parseUpdate(st));r["message"]=to_string(x)+" rows updated";return r;}throw runtime_error("Unsupported SQL statement: "+Parser::trim(st));}
